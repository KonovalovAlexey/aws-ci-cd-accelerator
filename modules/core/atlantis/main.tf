...
module "atlantis" {
  ...
  alb_ingress_cidr_blocks      = compact(concat(local.alb_ingress_cidr_blocks, var.allowed_cidr_blocks))
  alb_ingress_ipv6_cidr_blocks = []

  security_group_ids = [try(module.security-group[0].security_group_id, "")]
