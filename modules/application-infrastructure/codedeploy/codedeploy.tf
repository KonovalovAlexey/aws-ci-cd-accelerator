...
resource "aws_codedeploy_deployment_group" "ec2" {
  ...
  deployment_group_name  = "${var.repo_name}-${var.environments[count.index]}"
  service_role_arn       = var.codedeploy_role_create ? aws_iam_role.codedeploy_role[count.index].arn : var.codedeploy_role_arns[count.index]
